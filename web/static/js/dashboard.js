...
class RyxDashboard {
    constructor() {
        ...
        this.baseNodeUrl = this.detectBaseUrl();
        
        this.init();
    }

    detectBaseUrl() {
        // Try to detect a node URL from the current location
        const currentHost = window.location.hostname;
        const basePort = 8010; // Default HTTP port for first node
        return `http://${currentHost}:${basePort}`;
    }
    ...
    async discoverCluster() {
        ...
        try {
            ...
            await this.addNode(this.baseNodeUrl);
            
            // Discover other nodes through neighbors from status endpoint
            const status = await this.fetchFromNode(this.baseNodeUrl, '/status');
            if (status && status.neighbors) {
                for (const neighbor of status.neighbors) {
                    const nodeUrl = `http://${neighbor.address}:${neighbor.port - 1000}`; // HTTP port is UDP port - 1000
                    await this.addNode(nodeUrl);
                }
            }
    ...
    async addNode(nodeUrl) {
        try {
            const status = await this.fetchFromNode(nodeUrl, '/status');
            if (status) {
                this.nodes.set(nodeUrl, {
                    url: nodeUrl,
                    id: status.node_id || 'unknown',
                    status: 'healthy',
                    lastSeen: Date.now(),
                    data: status
                });
            ...
            console.warn(`Failed to add node ${nodeUrl}:`, error);
        }
    }

    async fetchFromNode(nodeUrl, endpoint) {
        try {
            ...
            console.warn(`Failed to fetch ${endpoint} from ${nodeUrl}:`, error);
            return null;
        }
    }

    async postToNode(nodeUrl, endpoint, data) {
        try {
            const response = await fetch(`${nodeUrl}${endpoint}`, {
                method: 'POST',
                headers: {
            ...
            console.warn(`Failed to post to ${endpoint} on ${nodeUrl}:`, error);
            return null;
        }
    }
    ...
    async updateAllNodes() {
        const promises = Array.from(this.nodes.keys()).map(async (nodeUrl) => {
    ...
    updateNodeGrid() {
        ...
        this.nodes.forEach((node, nodeUrl) => {
    ...
    async submitTask() {
        ...
        try {
            const result = await this.postToNode(healthyNode.url, '/compute', taskData);
            if (result) {
    ...
    async updateChemistryView() {
        ...
        try {
            const concentrations = await this.fetchFromNode(healthyNode.url, '/chemistry/concentrations');
            const stats = await this.fetchFromNode(healthyNode.url, '/chemistry/stats');
    ...
    async injectChemical() {
        ...
        try {
            const result = await this.postToNode(healthyNode.url, '/inject', chemicalData);
            if (result) {
...
Status: ${node.status}
URL: ${node.url}
Zone: ${node.data.spatial?.zone || 'unknown'}
Coordinate System: ${node.data.spatial?.coordinate_system || 'none'}
Neighbors: ${node.data.neighbor_count || 0}
Messages: ${node.data.message_count || 0}
Uptime: ${this.formatUptime(node.data.uptime || 0)}
        `;
        alert(details);
    }
